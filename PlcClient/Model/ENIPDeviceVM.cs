...
namespace PlcClient.Model
{
    ...
    public class ENIPDeviceVM
    {
        public static readonly Dictionary<uint, string> Vendors = new Dictionary<uint, string>()
        {
        ...
        public static readonly Dictionary<uint, string> Devices = new Dictionary<uint, string>()
        {
        ...
        [ReadOnly(true)]
        public byte ProductNameLength { get; private set; }
        public string ProductName { get; set; }
        public byte State { get; set; }

        public static string GetVendor(uint vendor_id)
        {
            if (Vendors.ContainsKey(vendor_id))
        ...
        public static ENIPDeviceVM Parse(byte[] data, string ipAddress = null)
        {
            ...
            {
                vm.IPAddress = new IPAddress(longip).ToString();
            }
            vm.VendorID = BitConverter.ToUInt16(data, 48);
            vm.Vendor = GetVendor(vm.VendorID);
            vm.DeviceID = BitConverter.ToUInt16(data, 50);
            vm.DeviceType = GetDevice(vm.DeviceID);
            vm.ProductCode = BitConverter.ToUInt16(data, 52);
            byte major = data[54];
            byte minor = data[55];
            vm.Revision = $"{major}.{minor}";
            vm.Status = BitConverter.ToUInt16(data, 56);
            vm.SerialNumber = "0x" + BitConverter.ToUInt32(data, 58).ToString("x");
            vm.ProductNameLength = data[62];
            vm.ProductName = System.Text.Encoding.UTF8.GetString(data, 63, vm.ProductNameLength);
            vm.State = data[data.Length - 1];
